#' Moving Sum
#'
#' Wrapper function for C++ code to evaluate a moving sum over each row for a matrix X. Equivalent to MATLAB's 'movsum' function.
#'
...
#'
#' @return Matrix of data with moving standard deviation corrected data
#'
#' @examples
#' set.seed(1234)
...
#' #evaluate a 5-point moving sum on each row of X:
#' X_msd <- movsum(X,3)
#'
#' If we load X into MATLAB, we can verify that it is equivalent (within machine tolerance) to:
